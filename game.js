...
return img;}
function update(){if(infrastructure()){updateInfrastructure();return;}const q=selection?buildingQuote():null;$('balance').textContent=short(state.money);$('balance').title=exact(state.money);$('count').textContent=state.buildings.length+' prédios · '+state.roads.length+' vias · '+state.parks.length+' parques';$('quote').hidden=!selection;
