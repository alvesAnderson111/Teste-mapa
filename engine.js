...
function smooth(raw,closed=false){let pts=[];for(const p of raw){if(validPoint(p)&&(!pts.length||distance(p,pts[pts.length-1])>.12))pts.push({x:p.x,y:p.y});}if(pts.length>160)pts=Array.from({length:160},(_,i)=>pts[Math.round(i*(pts.length-1)/159)]);if(closed&&pts.length>2&&distance(pts[0],pts.at(-1))<.15)pts.pop();for(let round=0;round<2;round++){if(pts.length<3)break;const next=closed?[]:[pts[0]];for(let i=0;i<(closed?pts.length:pts.length-1);i++){const a=pts[i],b=pts[(i+1)%pts.length];next.push({x:.75*a.x+.25*b.x,y:.75*a.y+.25*b.y},{x:.25*a.x+.75*b.x,y:.25*a.y+.75*b.y});}if(!closed)next.push(pts.at(-1));pts=next;}return pts;}
